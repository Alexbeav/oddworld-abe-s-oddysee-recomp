...
#
# Writes: dist/oddworld-abe-s-oddysee-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix oddworld-abe-s-oddysee-recomp \
  --exe-name Oddworld__Abe_s_Oddysee_Recompiled \
  --display-name "Oddworld: Abe's Oddysee Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Oddworld Abes Oddysee disc" \
